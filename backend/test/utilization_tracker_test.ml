...
(** Test fixtures *)
let test_db_config = Integration.Database.{
  host = "localhost";
  port = 5432;
...
let test_build_utilization () =
  Lwt_main.run (
    ...
    check bool "APY should be >= min" true (util.current_apy >= 6.5);
    check bool "APY should be <= max" true (util.current_apy <= 10.0);
...
let test_update_capital () =
  Lwt_main.run (
    ...
    let* util = UT.get_tranche_utilization ~tranche:SURE_MEZZ in
...
let test_update_coverage () =
  Lwt_main.run (
    ...
    (* Get updated utilization *)
    let* util = UT.get_tranche_utilization ~tranche:SURE_JNR in
...
let test_collateralization_ratio () =
  Lwt_main.run (
    ...
    let* ratio = UT.get_collateralization_ratio ~tranche:SURE_EQT in
    ...
    let* ratio_inf = UT.get_collateralization_ratio ~tranche:SURE_BTC in
    check bool "Infinite collateralization should be > 1000" true (ratio_inf > 1000.0);
...
let test_available_capacity () =
  Lwt_main.run (
    ...
    let* capacity = UT.get_available_capacity ~tranche:SURE_SNR in
...
let test_get_all_utilizations () =
  Lwt_main.run (
    ...
    let btc_util = List.find_exn all_utils ~f:(fun u ->
      equal_tranche u.tranche_id SURE_BTC
    ) in
    ...
    let snr_util = List.find_exn all_utils ~f:(fun u ->
      equal_tranche u.tranche_id SURE_SNR
    ) in
...
let test_cache_behavior () =
  Lwt_main.run (
    ...
    let* util1 = UT.get_tranche_utilization ~tranche:SURE_MEZZ in
    let timestamp1 = util1.last_updated in

    (* Second read immediately - should hit cache *)
    let* util2 = UT.get_tranche_utilization ~tranche:SURE_MEZZ in
    ...
    (* Invalidate cache *)
    UT.invalidate_cache SURE_MEZZ;

    (* Update and read again *)
    let* () = UT.update_capital ~tranche:SURE_MEZZ ~delta:(ton_to_nano 500.0) in
    let* util3 = UT.get_tranche_utilization ~tranche:SURE_MEZZ in
