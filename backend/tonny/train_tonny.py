...
# Configuration
BASE_MODEL = "mlx-community/Mistral-7B-Instruct-v0.2-4bit"  # Pre-quantized 4-bit version
MODELS_DIR = Path("./models")
ADAPTERS_DIR = Path("./adapters")
TRAINING_DATA = Path("./training_data")
OUTPUT_MODEL = "tonny-7b"

# Training hyperparameters (adjusted for 4-bit model)
TRAINING_CONFIG = {
    "iters": 500,  # Reduced for smaller model
    "learning_rate": 2e-5,  # Slightly higher for faster convergence
    "lora_layers": 16,
    "lora_rank": 8,
    "batch_size": 2,  # Smaller batches for 4-bit
    "warmup_steps": 50,
}
...
def download_base_model():
    """Download pre-quantized 4-bit model from MLX community"""
    print(f"📥 Downloading base model: {BASE_MODEL}")
    print("Using pre-quantized 4-bit model (~4GB)")

    mlx_model_path = MODELS_DIR / "mistral-7b-4bit"

    # Remove existing directory if it exists
    if mlx_model_path.exists():
        import shutil
        print(f"Removing existing directory: {mlx_model_path}")
        shutil.rmtree(mlx_model_path)

    mlx_model_path.mkdir(parents=True, exist_ok=True)

    # Download directly using huggingface_hub
    from huggingface_hub import snapshot_download

    print("Downloading from HuggingFace...")
    snapshot_download(
        repo_id=BASE_MODEL,
        local_dir=str(mlx_model_path),
...
def validate_training_data():
    ...
    training_file = TRAINING_DATA / "train" / "data.jsonl"
    print(f"🔍 Validating training data: {training_file}")
...
def fine_tune_model():
    ...
    cmd = [
        ...
        "--num-layers", str(TRAINING_CONFIG["lora_layers"]),  # Changed from --lora-layers
        "--adapter-path", str(adapter_path),  # Changed from --adapter-file
