...
import { escrowClient, EscrowContract, CreateEscrowRequest } from '../lib/escrow-client';
...
export const Escrow = () => {
  ...
  // Load user's escrows on mount and when user address changes
  useEffect(() => {
    if (!userAddress) {
      setEscrows([]);
      setLoading(false);
    ...
    const loadEscrows = async () => {
      try {
        setLoading(true);
        const response = await escrowClient.getUserEscrows(userAddress);
        setEscrows(response.escrows);
      } catch (error) {
        console.error('Failed to load escrows:', error);
        alert('Failed to load escrows. Check console for details.');
      } finally {
        setLoading(false);
      }
    };
  ...
  // Subscribe to real-time updates for active escrows
  useEffect(() => {
    ...
    escrows
      ...
      .forEach((escrow) => {
        const unsubscribe = escrowClient.subscribeToEscrow(escrow.escrow_id, (updatedEscrow) => {
          setEscrows((prev) =>
            prev.map((e) => (e.escrow_id === updatedEscrow.escrow_id ? updatedEscrow : e))
          );
        });
  ...
  // Calculate coverage revenue
  const totalCoverageRevenue = escrows
    .filter(e => e.protection_enabled && e.coverage_premium)
    .reduce((sum, e) => sum + (e.coverage_premium || 0), 0);

  const activeCoverageCount = escrows.filter(
    e => (e.status === 'active' || e.status === 'conditions_met') && e.protection_enabled
  ).length;
  ...
  return (
    <div className="space-y-8 pb-20">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
        ...
        <TerminalWindow title="COVERAGE_REVENUE">
          <div className="font-mono">
            <div className="text-3xl font-bold text-terminal-green">
              ${totalCoverageRevenue.toFixed(2)}
            </div>
            <div className="text-xs text-text-tertiary mt-1">
              From {activeCoverageCount} protected escrows
            </div>
          </div>
        </TerminalWindow>
                ...
                >
                  ...
                  <div className="flex items-center gap-2 text-xs font-mono text-text-tertiary">
                    ...
                    {escrow.protection_enabled && (
                      <span className="ml-auto text-copper-500 font-semibold">
                        🛡️ ${escrow.coverage_premium?.toFixed(2) || '0'}
                      </span>
                    )}
                  </div>
