...
export function sure_eqt_ConfigToCell(config: SURE_EQT_Config): Cell {
    return beginCell()
...
export class SURE_EQT implements Contract {
    constructor(
    ...
    static createFromConfig(config: SURE_EQT_Config, code: Cell, workchain = 0) {
        const data = sure_eqt_ConfigToCell(config);
        const init = { code, data };
        return new SURE_EQT(contractAddress(workchain, init), init);
    }
