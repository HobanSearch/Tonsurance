...
export function sure_jnr_ConfigToCell(config: SURE_JNR_Config): Cell {
    return beginCell()
...
export class SURE_JNR implements Contract {
    constructor(
    ...
    static createFromConfig(config: SURE_JNR_Config, code: Cell, workchain = 0) {
        const data = sure_jnr_ConfigToCell(config);
        const init = { code, data };
        return new SURE_JNR(contractAddress(workchain, init), init);
    }
