...
export type PolicyRouterConfig = {
    ...
    paused: boolean;
    shardAddresses: Cell | null; // Dict: uint8 -> address (256 shards)
...
export class PolicyRouter implements Contract {
     ...
     */
    async getShardAddress(provider: ContractProvider, shardId: number): Promise<Address> {
     ...
     */
    async getShardForPolicy(provider: ContractProvider, policyId: bigint): Promise<{
        shardId: number;
        shardAddress: Address;
    }> {
