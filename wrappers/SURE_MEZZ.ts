...
export function sure_mezz_ConfigToCell(config: SURE_MEZZ_Config): Cell {
    return beginCell()
...
export class SURE_MEZZ implements Contract {
    constructor(
    ...
    static createFromConfig(config: SURE_MEZZ_Config, code: Cell, workchain = 0) {
        const data = sure_mezz_ConfigToCell(config);
        const init = { code, data };
        return new SURE_MEZZ(contractAddress(workchain, init), init);
    }
