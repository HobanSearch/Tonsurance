...
export type OracleSubFactoryConfig = {
...
export function oracleSubFactoryConfigToCell(config: OracleSubFactoryConfig): Cell {
    return beginCell()
...
// Oracle Provider Asset IDs
export const ASSET_REDSTONE = 1;
export const ASSET_PYTH = 2;
export const ASSET_CHAINLINK = 3;
export const ASSET_DIA = 4;

export class OracleSubFactory implements Contract {
    constructor(
    ...
    static createFromAddress(address: Address) {
        return new OracleSubFactory(address);
    }

    static createFromConfig(config: OracleSubFactoryConfig, code: Cell, workchain = 0) {
        const data = oracleSubFactoryConfigToCell(config);
        const init = { code, data };
        return new OracleSubFactory(contractAddress(workchain, init), init);
    }
    ...
    async getSupportedOracles(provider: ContractProvider): Promise<number> {
        const result = await provider.get('get_supported_oracles', []);
        return Number(result.stack.readBigNumber());
    }
