...
export type TradFiNatCatFactoryConfig = {
...
export function tradFiNatCatFactoryConfigToCell(config: TradFiNatCatFactoryConfig): Cell {
    return beginCell()
...
// Natural Catastrophe Asset IDs
export const ASSET_HURRICANE = 1;
export const ASSET_EARTHQUAKE = 2;

export class TradFiNatCatFactory implements Contract {
    constructor(
    ...
    static createFromAddress(address: Address) {
        return new TradFiNatCatFactory(address);
    }

    static createFromConfig(config: TradFiNatCatFactoryConfig, code: Cell, workchain = 0) {
        const data = tradFiNatCatFactoryConfigToCell(config);
        const init = { code, data };
        return new TradFiNatCatFactory(contractAddress(workchain, init), init);
    }
    ...
    async getSupportedCatastrophes(provider: ContractProvider): Promise<number> {
        const result = await provider.get('get_supported_catastrophes', []);
        return Number(result.stack.readBigNumber());
    }
