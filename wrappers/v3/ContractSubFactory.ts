...
export type ContractSubFactoryConfig = {
...
export function contractSubFactoryConfigToCell(config: ContractSubFactoryConfig): Cell {
    return beginCell()
...
// DeFi Protocol Asset IDs
export const ASSET_DEDUST = 1;
export const ASSET_STONFI = 2;
export const ASSET_TONSTAKERS = 3;
export const ASSET_EVAA = 4;

export class ContractSubFactory implements Contract {
    constructor(
    ...
    static createFromAddress(address: Address) {
        return new ContractSubFactory(address);
    }

    static createFromConfig(config: ContractSubFactoryConfig, code: Cell, workchain = 0) {
        const data = contractSubFactoryConfigToCell(config);
        const init = { code, data };
        return new ContractSubFactory(contractAddress(workchain, init), init);
    }
    ...
    async getSupportedProtocols(provider: ContractProvider): Promise<number> {
        const result = await provider.get('get_supported_protocols', []);
        return Number(result.stack.readBigNumber());
    }
