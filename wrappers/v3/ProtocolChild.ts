...
export type ProtocolChildConfig = {
    parentFactoryAddress: Address;
    masterFactoryAddress: Address;
    productType: number; // = 4 (PRODUCT_CONTRACT)
    assetId: number;
    policyNFTMinterAddress: Address;
    vaultAddress: Address;
    protocolMonitorAddress: Address;
...
export function protocolChildConfigToCell(config: ProtocolChildConfig): Cell {
    ...
    const addrRef1 = beginCell()
        ...
        .storeAddress(config.protocolMonitorAddress)
        .endCell();
...
export const BASE_APR_BPS = 200; // 2.0% annual (highest risk)
export const PROTOCOL_PAUSE_THRESHOLD_SECONDS = 86400; // 24 hours

export class ProtocolChild implements Contract {
    constructor(readonly address: Address, readonly init?: { code: Cell; data: Cell }) {}

    static createFromAddress(address: Address) {
        return new ProtocolChild(address);
    }

    static createFromConfig(config: ProtocolChildConfig, code: Cell, workchain = 0) {
        const data = protocolChildConfigToCell(config);
        const init = { code, data };
        return new ProtocolChild(contractAddress(workchain, init), init);
    }
    ...
    async sendSetProtocolMonitor(provider: ContractProvider, via: Sender, opts: { value: bigint; monitorAddress: Address }) {
        await provider.internal(via, {
            value: opts.value,
            sendMode: SendMode.PAY_GAS_SEPARATELY,
    ...
    async getProtocolMonitor(provider: ContractProvider): Promise<Address> {
        const result = await provider.get('get_protocol_monitor', []);
        return result.stack.readAddress();
    }
    ...
    async getPauseThreshold(provider: ContractProvider): Promise<number> {
        const result = await provider.get('get_pause_threshold', []);
        return Number(result.stack.readBigNumber());
    }
