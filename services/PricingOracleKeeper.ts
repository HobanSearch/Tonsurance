...
export interface PricingOracleKeeperConfig {
    ...
    updateIntervalSeconds?: number; // Default: 5 seconds
}
...
export class PricingOracleKeeper {
    ...
    private config: PricingOracleKeeperConfig;
     ...
     */
    async fetchOCamlPrice(asset: string): Promise<OCamlConsensusPrice> {
        try {
            const response = await axios.get(
                `${this.config.ocamlBackendUrl}/api/v1/consensus-price/${asset}`,
                { timeout: 3000 }
            );
     ...
     */
    async fetchPolymarketOdds(coverageType: CoverageType): Promise<number> {
        ...
        try {
            ...
            const mockOdds = {
                [CoverageType.DEPEG]: 250,    // 2.5% - USDT depeg probability
                [CoverageType.EXPLOIT]: 150,  // 1.5% - Smart contract hack
                [CoverageType.BRIDGE]: 300,   // 3.0% - Bridge exploit
            };
     ...
     */
    async fetchPerpFundingRate(asset: string): Promise<number> {
     ...
     */
    async fetchAllianzQuote(coverageType: CoverageType): Promise<number> {
        ...
        try {
            ...
            const mockQuotes = {
                [CoverageType.DEPEG]: 450,    // $4.50 per $1000
                [CoverageType.EXPLOIT]: 600,  // $6.00 per $1000
                [CoverageType.BRIDGE]: 550,   // $5.50 per $1000
            };
     ...
     */
    async fetchHedgeCosts(coverageType: CoverageType): Promise<HedgeCosts> {
     ...
     */
    async updateOracleOnChain(
        coverageType: CoverageType,
        hedgeCosts: HedgeCosts
    ): Promise<void> {
        try {
            ...
            await this.oracle.sendUpdateHedgePrices(
                this.client.provider(this.oracle.address),
                this.keeperWallet.sender(),
                {
    ...
    async runKeeperLoop(): Promise<void> {
        ...
        while (this.running) {
            ...
            try {
                ...
                const depegCosts = await this.fetchHedgeCosts(CoverageType.DEPEG);
                const exploitCosts = await this.fetchHedgeCosts(CoverageType.EXPLOIT);
                const bridgeCosts = await this.fetchHedgeCosts(CoverageType.BRIDGE);

                // Update on-chain oracle (3 transactions)
                await this.updateOracleOnChain(CoverageType.DEPEG, depegCosts);
                await this.updateOracleOnChain(CoverageType.EXPLOIT, exploitCosts);
                await this.updateOracleOnChain(CoverageType.BRIDGE, bridgeCosts);
    ...
    async healthCheck(): Promise<{
        ...
        try {
            // Check OCaml backend
            const ocamlHealthy = await axios
